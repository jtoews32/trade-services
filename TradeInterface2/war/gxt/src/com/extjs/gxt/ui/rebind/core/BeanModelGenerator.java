...
public class BeanModelGenerator extends Generator {
  ...
  public String generate(TreeLogger logger, GeneratorContext context, String typeName) throws UnableToCompleteException {
    ...
    try {
      ...
      if (pw != null) {
        ...
        sw.println("private Map<String, BeanModelFactory> map;");

        sw.println("public BeanModelFactory getFactory(Class beanClass) {");
        sw.indent();
        sw.println("if (map == null) {");
        sw.indentln("map = new FastMap<BeanModelFactory>();");
        sw.println("}");
        sw.println("BeanModelFactory factory = map.get(beanClass.getName());");
        ...
        for (JClassType bean : beans) {
          ...
          sw.println("if (beanClass.getName().equals(\"" + bean.getQualifiedSourceName() + "\")) {");
          sw.indent();
          sw.println("factory = new " + factory + "();");
          sw.println("map.put(beanClass.getName(), factory);");
          sw.outdent();
          sw.print("}");
  ...
  protected void createGetMethods(List<JMethod> getters, SourceWriter sw, String typeName) {
    ...
    for (JMethod method : getters) {
      ...
      try {
        if (returnType != null) {
          if (returnType.isAssignableTo(oracle.getType(List.class.getName()))) {
